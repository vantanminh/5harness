...
The advisory result from `harness score-context <trace-id>`.
...
`harness query tools` and documented in `docs/TOOL_REGISTRY.md`.
...
The drift score from `harness audit`. Lower scores mean fewer
orphaned, stale, unverified, outcome-missing, or broken-tool records.

## Improvement Proposal

A structured proposal generated by `harness propose` from
