...
const VALID_ROLES: &[&str] = &[
    "frontend", "backend", "mobile", "service", "shared", "other",
];

pub fn set_role(project_root: &Path, role: &str, stack: Option<&str>) -> Result<Value> {
    if !VALID_ROLES.contains(&role) {
        return Err(Error::new(format!(
            "invalid project role {role}; expected one of {}",
            VALID_ROLES.join(", ")
        )));
    }
...
pub fn role(project_root: &Path) -> Result<Value> {
    ...
    Ok(
        json!({"role": marker_value(&text, "harness-project-role"), "stack": marker_value(&text, "harness-project-stack").map(|v| v.split(',').map(str::to_string).collect::<Vec<_>>())}),
    )
}
...
pub fn add_peer(project_root: &Path, id_or_path: &str, role: Option<&str>) -> Result<Value> {
    ...
    let reverse = if let Some(ref path) = peer_path {
        if let Ok(peer_agents) = fs::read_to_string(path.join("AGENTS.md")) {
            ...
            if let Ok(updated_peer) =
                replace_markers(&peer_agents, None, None, Some(&reverse_marker), false)
            {
    ...
    Ok(
        json!({"id":peer_id,"role":role.unwrap_or("other"),"path":peer_path,"reverse_written":reverse}),
    )
}
...
pub fn resolve_peer(
    ...
    let selected = if let Some(peer) = peer {
        configured
            .into_iter()
            .find(|p| p["id"].as_str() == Some(peer) || p["role"].as_str() == Some(peer))
    } else if let Some(role) = role {
        let matches: Vec<_> = configured
            .into_iter()
            .filter(|p| p["role"].as_str() == Some(role))
            .collect();
        if matches.len() > 1 {
            return Err(Error::new(format!(
                "peer role {role} is ambiguous; choose --peer"
            )));
        }
    ...
    let path = selected["path"]
        .as_str()
        .ok_or_else(|| Error::new("peer is not registered on this machine"))?;
    let path = PathBuf::from(path);
    if !path.is_dir() {
        return Err(Error::new(format!(
            "peer path is missing: {}",
            path.display()
        )));
    }
    Ok(path)
...
pub fn ensure_peer_write_allowed(target: &Path) -> Result<()> {
    let Some(raw) = std::env::var_os("HARNESS_PEER_WRITE_ROOTS") else {
        return Ok(());
    };
        ...
        .filter(|v| !v.is_empty())
    {
        ...
        if !root.is_absolute() || !root.is_dir() {
            return Err(Error::new(
                "HARNESS_PEER_WRITE_ROOTS must contain existing absolute directories",
            ));
        }
    ...
    if roots.iter().any(|root| canonical_target.starts_with(root)) {
        ...
        Err(Error::new(format!(
            "peer report target is outside HARNESS_PEER_WRITE_ROOTS: {}",
            target.display()
        )))
    }
}
...
fn resolve_project(id_or_path: &str) -> Result<(String, Option<PathBuf>)> {
    ...
    let project = registry
        .projects
        .into_iter()
        .find(|p| p.id == id_or_path)
        .ok_or_else(|| Error::new(format!("project {id_or_path} is not registered")))?;
    Ok((project.id, Some(PathBuf::from(project.path))))
}

fn extract_local_id(project_root: &Path) -> Result<String> {
    extract_project_id(&fs::read_to_string(project_root.join("AGENTS.md"))?)
        .ok_or_else(|| Error::new("project has no durable harness id"))
}
...
fn parse_stack(stack: Option<&str>) -> Result<Vec<String>> {
        ...
        .filter(|s| !s.is_empty())
    {
        ...
        if out.iter().any(|v| v == raw) {
            return Err(Error::new(format!("duplicate stack tag {raw}")));
        }
        if out.len() == 4 {
            return Err(Error::new("at most four stack tags are allowed"));
        }
...
fn remove_peer_marker(text: &str, id: &str) -> String {
    text.lines()
        .filter(|line| !(line.contains("harness-peer:") && line.contains(&format!("id={id};"))))
        .collect::<Vec<_>>()
        .join("\n")
        + if text.ends_with('\n') { "\n" } else { "" }
}
...
fn replace_markers(
    ...
    let block = extract_harness_block(text)
        .ok_or_else(|| Error::new("AGENTS.md has no harness-managed block"))?;
    ...
    if let Some(role) = role {
        ...
        let idx = lines
            .iter()
            .position(|line| line.contains("harness-project-id:"))
            .map(|i| i + 1)
            .unwrap_or(1);
    ...
    if let Some(stack) = stack {
        ...
        if !stack.is_empty() {
            let idx = lines
                .iter()
                .position(|line| line.contains("harness-project-role:"))
                .map(|i| i + 1)
                .or_else(|| {
                    lines
                        .iter()
            ...
            lines.insert(
                idx,
                format!("<!-- harness-project-stack: {} -->", stack.join(",")),
            );
        }
    }
    if let Some(peer) = peer {
        if peer_replace
            || !lines.iter().any(|line| {
                line.contains("harness-peer:")
                    && line.contains(peer.split(';').next().unwrap_or(peer))
            })
        {
            let idx = lines
                .iter()
                .position(|line| line.contains("harness-project-stack:"))
                .map(|i| i + 1)
                .or_else(|| {
                    lines
                        .iter()
    ...
    Ok(format!(
        "{}{}{}",
        block.before_marker(text),
        block,
        block.after_marker(text)
    ))
}
