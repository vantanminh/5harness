...
export function queryReportsMd(
  projectRoot: string,
  catalog?: ProjectCatalog,
): string {
  const cat = catalog ?? buildCatalog(projectRoot);
  const rows = [...cat.byType.report]
    .sort((a, b) => a.id.localeCompare(b.id))
    .map((e) => ({
      id: e.id,
      status: e.status,
      severity: asString(e.data, "severity") ?? "",
      summary: asString(e.data, "summary") ?? e.title,
      updated_at: asString(e.data, "updated_at") ?? "",
    }));
  return formatTable(rows, [
    "id",
    "status",
...
/** Structured query payload for `--json` and MCP (US-073 / US-076). */
export function queryViewData(
  projectRoot: string,
  view: string,
  options: { numeric?: boolean; open?: boolean; closed?: boolean } = {},
): unknown {
  const cat = buildCatalog(projectRoot);
  switch (view) {
    case "matrix":
      return cat.byType.story.map((e) => ({
        id: e.id,
        title: e.title,
        status: e.status,
    ...
    case "stats":
      return {
        ...
        backlog_items: cat.byType.backlog.length,
        reports: cat.byType.report.length,
        traces: listLocalTraces(projectRoot).length,
      };
    case "intakes":
      return [...cat.byType.intake]
        .sort((a, b) => b.id.localeCompare(a.id))
        .slice(0, 50)
        .map((e) => ({
          id: e.id,
    ...
    case "decisions":
      return cat.byType.decision.map((e) => ({
        id: e.id,
        title: e.title,
        status: e.status,
        doc: asString(e.data, "doc") ?? e.path,
    ...
    case "backlog": {
      ...
      return [...items].sort((a, b) => b.id.localeCompare(a.id)).map((e) => ({
        id: e.id,
        title: e.title,
        risk: asString(e.data, "risk")
          ? laneDisplay(asString(e.data, "risk")!)
          : "",
        status: e.status,
        predicted: asString(e.data, "predicted") ?? "",
        outcome: asString(e.data, "outcome") ?? "",
    ...
    case "traces":
      return [...listLocalTraces(projectRoot)]
        .sort((a, b) => b.id - a.id)
        .slice(0, 50)
        .map((r) => ({
          id: r.id,
